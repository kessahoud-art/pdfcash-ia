...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Simple text line, no filled rectangle
    // ════════════════════════════════════════
    function addHeader(docTitle, section) {
      ...
      return 32; // y starts after header
    }
    ...
    // ════════════════════════════════════════
    // PAGE 1 — COVER (only colored page)
    // ════════════════════════════════════════
    doc.addPage();
    pageNum++;
    ...
    // Decorative elements — circles with opacity
    // Large circle top right
    doc.circle(W + 20, -20, 200)
      .fillOpacity(0.12).fill(rgb(lgt(AC, 60))).fillOpacity(1);
    // Small circle bottom left
    doc.circle(-30, H + 30, 140)
      .fillOpacity(0.10).fill(rgb(lgt(AC, 40))).fillOpacity(1);
    // Medium circle middle right
    doc.circle(W - 30, H / 2, 80)
      .fillOpacity(0.07).fill(rgb([255, 255, 255])).fillOpacity(1);

    // Top accent bar (slightly lighter)
    doc.rect(0, 0, W, 6).fill(rgb(lgt(AC, 40)));

    // Bottom bar
    doc.rect(0, H - 40, W, 40).fill(rgb(dkn(BG, 30)));

    // Cover footer text
    doc.fillColor(rgb(lgt(AC, 120))).font("Helvetica").fontSize(9);
    doc.text("Par " + author, M, H - 26, { lineBreak: false });
    doc.fillColor(rgb([255, 255, 255])).font("Helvetica-Bold").fontSize(9);
    doc.text("PDF Cash IA", W - M - 65, H - 26, { lineBreak: false });

    // Content starts after top bar
    let cy = 30;

    // Document type label
    doc.fillColor(rgb(lgt(AC, 130))).font("Helvetica-Bold").fontSize(10);
    doc.text(
    ...
    doc.moveTo(M, cy).lineTo(M + 40, cy).lineWidth(2).strokeColor(rgb([255, 255, 255])).stroke();
    cy += 14;

    // Main title — large, white, bold
    const titleText = clean(C.title);
    doc.fillColor(rgb([255, 255, 255])).font("Helvetica-Bold").fontSize(30);
    ...
    if (C.subtitle) {
      ...
      doc.fillColor(rgb(lgt(AC, 140))).font("Helvetica").fontSize(13);
    ...
    // Tagline in italic
    if (C.tagline) {
      doc.fillColor(rgb([255, 255, 255])).fillOpacity(0.75)
        .font("Helvetica-Oblique").fontSize(12);
      const tagText = '"' + clean(C.tagline) + '"';
      const tagh = doc.heightOfString(tagText, { width: TW - 20 });
      doc.text(tagText, M, cy, { width: TW - 20 });
      doc.fillOpacity(1);
      cy += tagh + 20;
    }

    // Chapter count info
    doc.fillColor(rgb(lgt(AC, 120))).font("Helvetica").fontSize(10);
    doc.text(
    ...
    // TOC items — no filled backgrounds
    ...
    (C.chapters || []).forEach((ch, ci) => {
      ...
      // Widow prevention: heading + first few lines must fit
      ...
      // Chapter title — accent color, large, no box
    ...
    (C.key_takeaways || []).forEach(k => {
      ...
      // Left accent line (no filled box)
    ...
    // CTA — bordered box (lines only, no fill)
